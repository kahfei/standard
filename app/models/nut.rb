class Nut < ActiveRecord::Base
   ...
   include CommonConstants
    ...
    self.per_page = 15
  	...
  	def self.search(query)
        ...
        or nut_type LIKE ? or nut_size LIKE ? or width LIKE ? or drawing_norm LIKE ? 
  ...
  def self.accessible_attributes
    ["id","partnumber","description","nut_type","nut_size","width",
    	"drawing_norm","coating",
  ...
  scope :sorted_by, lambda { |sort_option|
    ...
    when /^drawing_norms_/
    order("LOWER(nuts.drawing_norm) #{ direction }")
...
scope :search_query, lambda { |query|
  ...
  where("LOWER(nuts.partnumber) LIKE ? OR LOWER(nuts.description) LIKE ? OR LOWER(nuts.nut_type) LIKE ?
          OR LOWER(nuts.nut_size) LIKE ? OR LOWER(nuts.width) LIKE ? 
          OR LOWER(nuts.coating) LIKE ?
           OR LOWER(nuts.drawing_norm) LIKE ? OR LOWER(nuts.where_used_product) LIKE ?
