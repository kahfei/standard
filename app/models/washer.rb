class Washer < ActiveRecord::Base
  ...
  include CommonConstants
    ...
    self.per_page = 15
  	...
  	def self.search(query)
        ...
        or thickness LIKE ? or coating LIKE ? or material LIKE ? or drawing_norm LIKE ? 
  ...
  def self.accessible_attributes
    ["id","partnumber","description","washer_type","inner_diameter","outer_diameter",
    	"thickness", "coating", "material", "drawing_norm",
  ...
  scope :sorted_by, lambda { |sort_option|
    ...
    when /^drawing_norm_/
    order("LOWER(washers.drawing_norm) #{ direction }")
...
scope :search_query, lambda { |query|
  ...
  where("LOWER(washers.partnumber) LIKE ? OR LOWER(washers.description) LIKE ? OR LOWER(washers.washer_type) LIKE ?
          OR LOWER(washers.inner_diameter) LIKE ? OR LOWER(washers.outer_diameter) LIKE ?
           OR LOWER(washers.thickness) LIKE ? OR LOWER(washers.coating) LIKE ? OR LOWER(washers.material) LIKE ?
           OR LOWER(washers.drawing_norm) LIKE ? OR LOWER(washers.where_used_product) LIKE ? OR LOWER(washers.where_used_plant) LIKE ?
            OR LOWER(washers.remark) LIKE ? OR LOWER(washers.reason) LIKE ?",
